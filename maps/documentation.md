...
#     Creates the actual schedule using the scores and distances between attractions. Also calculates the time of travel between each attraction.
# @param:
...
# @return:
#     compile_schedule (list): a list of attractions representing a day
#     tpa (int): the time to spend at each attraction
#     tpr (int): the time to spend at each restaurant
```
...
# @return:
#     attraction_list_clusters (list): at list of attraction clusters
#     centroids (list): a list representing a centroid
